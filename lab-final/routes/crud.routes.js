...
// =======================================================
// ✅ ADD TO CART (SESSION BASED) - NEW ROUTES
// =======================================================
...
router.post("/cart/add/:id", async (req, res) => {
  try {
    ...
    cart.items[productId].subtotal =
      cart.items[productId].qty * cart.items[productId].product.price;
    ...
    return res.status(500).send("Add to cart failed: " + err.message);
  }
});
...
router.get("/cart", (req, res) => {
  ...
  res.render("checkout", { cart, cartItems }); // ✅ reusing checkout page as cart page
});

// ✅ Update quantity (inc/dec)
// POST /crud/cart/update
router.post("/cart/update", (req, res) => {
  const { productId, action } = req.body;

  const cart = req.session.cart;
  if (!cart || !cart.items[productId]) return res.redirect("/crud/cart");
  ...
  if (cart.items[productId]) {
    cart.items[productId].subtotal = cart.items[productId].qty * price;
  }
  ...
  return res.redirect("/crud/cart");
});

// ✅ Remove item completely
// POST /crud/cart/remove/:id
